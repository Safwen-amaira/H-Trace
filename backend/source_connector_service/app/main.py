...
app = FastAPI(title="H-Trace Source Connector Service", version="1.0.0")

logging.basicConfig(level=logging.INFO)

@app.get("/sources")
def get_available_sources(plan: Optional[str] = Query("free")):
...
async def fetch_and_store(plan: str = "free"):
    ...
    async with httpx.AsyncClient() as client:
        for source_name in allowed:
            url = THREAT_SOURCES[source_name]
            try:
                # Simulated: generate a mock IOC
                mock_ioc = {
                    "type": "domain",
                ...
                resp = await client.post(f"{THREAT_INTEL_SERVICE_URL}/iocs", json=mock_ioc)
